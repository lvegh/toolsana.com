...
// MINIMAL CORS - only for AI routes
const aiCors = cors({
  origin: true, // Allow all origins for AI processing
  methods: ['GET', 'POST'],
  allowedHeaders: ['Content-Type', 'Authorization', 'X-API-Key'],
  credentials: true
});

// Apply minimal CORS only
router.use(aiCors);
...
// MINIMAL multer configuration - no complex validation
...
// BACKGROUND REMOVAL - MINIMAL MIDDLEWARE STACK
router.post('/remove-background', 
    basicRateLimit,  // ONLY basic rate limiting
...
// Device capability assessment endpoint - MINIMAL MIDDLEWARE
router.post('/check-device-capability', basicRateLimit, async (req, res) => {
    try {
        ...
        // Calculate capability score (simplified version)
...
// API information endpoint - NO MIDDLEWARE
